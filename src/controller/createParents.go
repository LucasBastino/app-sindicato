...
func CreateParents(c *fiber.Ctx) error {
	...
	result, err := database.DB.Query("SELECT IdMember, Birthday FROM MemberTable WHERE CAST(Birthday AS DATE) > '1974-01-01'")
	if err != nil {
	...
	for result.Next() {
		err = result.Scan(&memberDateInfo.id, &memberDateInfo.birthday)
		if err != nil {
