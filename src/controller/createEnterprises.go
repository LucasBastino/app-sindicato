...
func CreateEnterprises(c *fiber.Ctx) error {
	...
	for i := 0; i < 50; i++ {
		...
		e.Number = strconv.Itoa(rand.IntN(8000) + 1000)
		...
		insert, err := database.DB.Query(fmt.Sprintf("INSERT INTO EnterpriseTable (Name, Number, Address, CUIT, District, PostalCode, Phone) VALUES ('%s','%s','%s','%s','%s','%s', '%s')", e.Name, e.Number, e.Address, e.CUIT, e.District, e.PostalCode, e.Phone))
		if err != nil {
