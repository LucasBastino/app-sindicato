...
func CreateEnterprises(c *fiber.Ctx) error {
	...
	for i := 0; i < 50; i++ {
		...
		insert, err := database.DB.Query(fmt.Sprintf("INSERT INTO EnterpriseTable (Name, Address, CUIT, District, PostalCode, Phone) VALUES ('%s','%s','%s','%s','%s','%s')", e.Name, e.Address, e.CUIT, e.District, e.PostalCode, e.Phone))
		if err != nil {
